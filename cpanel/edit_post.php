...
if (isset($_POST["upost"])) 
{
    ...
    $keyword = htmlentities($_POST["tags"]);
    $prices = $_POST["price"];
    ...
    $tag = $_POST["tag"];
    $times = time();
    $cv_time = date("Y-m-d H:i:s",$times);
    $sql = "UPDATE posts SET title = '$title',videoreviews = '$videoreviews',updatedate = '$cv_time', prices = $prices, img = '$img', author = '$author', content = '$content', tutorials = '$tutorials', linkdown = '$linkdown', linkdemo ='$linkdemo', tag = $categories, keyword = '$keyword' where id = $idpost";
     ...
     echo '<script>$(function () {toastr.error("An error occurred", "Error")});</script>';
        ...
        if (isset($_GET["gameid"])) 
        {
            ...
            while ( $data = $csdl->fetch_array($query) ) 
            {
                ...
                $reactnum = $data["reactnum"];
                $quecate = "select c.cateid,c.catename FROM categories c,posts v WHERE ".$data["tag"]." = c.cateid group by c.cateid,c.catename";
                $result3= $csdl->query($quecate);
                $fcate = $csdl->fetch($result3);
                $tag_string = $fcate["catename"];
            }
        }	
?>
          ...
          <div class="card bg-secondary shadow">
            <div class="card-header bg-white border-0">
              <div class="row align-items-center">
                <div class="col-8">
                  <h3 class="mb-0">Update - <?php echo $title;?></h3>
                </div>
                <div class="col-4 text-right">
                  <span class="btn btn-sm btn-primary"><?php echo $author;?></span>
                </div>
              </div>
            </div>
            <div class="card-body">
            ...
            <div class="card-header bg-white border-0">
              <div class="row align-items-center">
                ...
                <div class="col-4 text-right">
                  <input type="submit" name="upost" value="UPDATE POST" class="btn btn-sm btn-warning">
                </div>
              </div>
